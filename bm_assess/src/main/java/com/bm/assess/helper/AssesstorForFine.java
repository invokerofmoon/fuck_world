...
public class AssesstorForFine extends Assesstor {
    @Override
...
//        代码bug率
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(2l)).collect(Collectors.toList()));
        this.param.put("codeBugRateScore", describeAndScore.getScore());
        this.param.put("codeBugRateMsg", describeAndScore.getDescribeInfo());
//        代码执行效率
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(4l)).collect(Collectors.toList()));
...
//        代码可读性
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(5l)).collect(Collectors.toList()));
...
//        文档质量
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(6l)).collect(Collectors.toList()));
...
//        工作完成情况
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(7l)).collect(Collectors.toList()));
        this.param.put("workFinishScore", describeAndScore.getScore());
        this.param.put("workFinishMsg", describeAndScore.getDescribeInfo());
//        专业知识
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(8l)).collect(Collectors.toList()));
        this.param.put("knowledgeScore", describeAndScore.getScore());
        this.param.put("knowledgeMsg", describeAndScore.getDescribeInfo());
//        发现解决问题能力
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(9l)).collect(Collectors.toList()));
...
//        分析判断能力
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(10l)).collect(Collectors.toList()));
...
//        创新能力
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(14l)).collect(Collectors.toList()));
        this.param.put("innovateScore", describeAndScore.getScore());
        this.param.put("innovateMsg", describeAndScore.getDescribeInfo());
//        遵章守纪
        describeAndScore = super.randomGetOneFromList(describeAndScoreList.stream().filter(des -> des.getItemId().equals(15l)).collect(Collectors.toList()));
    ...
    public AssesstorForFine(Integer minStore, Integer maxStore, Integer offDays, Map<String, Object> param, Map<String, Object> assessConfigInfo) {
